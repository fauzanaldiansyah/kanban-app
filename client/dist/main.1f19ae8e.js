...
function editTask(event) {
  ...
  $.ajax({
    method: 'put',
    url: URL + "tasks/".concat(kanbanCurrentId),
    headers: {
      access_token: localStorage.access_token
    },
    data: {
...
function getTask() {
  $.ajax({
    ...
    data.forEach(function (element) {
      $('#tasks-data').append("\n                    <tr id = 'tasks-row'>\n                        <td class=\"font-weight-bold\">  ".concat(element.title, " </td>\n                        <td> ").concat(element.category, " </td>\n                        <td> <button class=\"btn btn-secondary btn-sm\" onclick=\"editFormPage(").concat(element.id, ")\"> Edit  </button> \n                            <button class=\"btn btn-secondary btn-sm\" onclick=\"deleteTask(").concat(element.id, ")\"> Delete  </button> \n                        </td>\n                    </tr>\n            "));
    });
  }).fail(function (err) {
    console.log(err);
  });
...
function registerUser(event) {
  ...
  var organization = $('#organization-register').val();
  $.ajax({
    method: 'post',
